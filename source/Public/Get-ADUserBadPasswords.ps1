<#
.SYNOPSIS
    Gets bad password attempts
.DESCRIPTION
    Long description
.EXAMPLE
    PS C:\> Get-ADUserBadPasswords -StartTime (Get-Date).AddDays(-1) -Identity tonyb -DomainController DC0001
    Gets bad passwords for a user within the last day on DC DC0001
.EXAMPLE
    PS C:\> Get-ADUserBadPasswords -StartTime (Get-Date).AddDays(-1)
    Gets all bad passwords, by default it checks on the PDC if you don't specify a DC
.INPUTS
    Inputs (if any)
.OUTPUTS
    Output (if any)
